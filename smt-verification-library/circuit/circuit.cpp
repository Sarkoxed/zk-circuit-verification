 ...
 */
Circuit::Circuit(CircuitSchema& circuit_info, Solver* solver, TermType type, const std::string& tag, bool optimizations, bool isPLONK)
...
size_t Circuit::prepare_gates(size_t cursor)
{
    if(isPLONK){
        ...
        std::string q_m = this->selectors[cursor][0][0];
        std::string q_1 = this->selectors[cursor][0][1];
        std::string q_2 = this->selectors[cursor][0][2];
        std::string q_3 = this->selectors[cursor][0][3];
        std::string q_c = this->selectors[cursor][0][4];

        uint32_t w_l = this->wires_idxs[cursor][0][0];
        uint32_t w_r = this->wires_idxs[cursor][1][0];
        uint32_t w_o = this->wires_idxs[cursor][2][0];
        optimized[w_l] = false;
        optimized[w_r] = false;
        optimized[w_o] = false;
        ...
        STerm A = symbolic_vars[0];
        STerm B = symbolic_vars[0];
        STerm C = symbolic_vars[0];
        for(size_t j = 0; j < this->selectors[cursor][0].size(); j++){
            A += symbolic_vars[this->wires_idxs[cursor][0][j]] * this->selectors[cursor][0][j];
        }
        for(size_t j = 0; j < this->selectors[cursor][1].size(); j++){
            B += symbolic_vars[this->wires_idxs[cursor][1][j]] * this->selectors[cursor][1][j];
        }
        for(size_t j = 0; j < this->selectors[cursor][2].size(); j++){
            C += symbolic_vars[this->wires_idxs[cursor][2][j]] * this->selectors[cursor][2][j];
        }
        A * B -C == symbolic_vars[0];
    }
    return cursor + 1;
}
