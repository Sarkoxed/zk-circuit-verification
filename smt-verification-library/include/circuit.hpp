...
//    void init_graph();
    void add_gates();
    void prepare_gates();
//    void quadratic_polynomial_handler(barretenberg::fr q_m, barretenberg::fr q_1, barretenberg::fr q_2, barretenberg::fr q_3, barretenberg::fr q_c, uint32_t w);
  ...
  public:
    ...
    std::vector<std::vector<std::vector<std::string>>> selectors;             // selectors from the circuit TODO: currently only R1CS
    std::vector<std::vector<std::vector<uint32_t>>> wires_idxs;                    // values of the gates' wires
    std::vector<FF> symbolic_vars;                                    // all the symbolic variables from the circuit
    //std::vector<std::vector<uint32_t>> graph; TODO: translate to r1cs
    ...
    explicit Circuit(nlohmann::json& circuit_info, Solver* solver, const std::string& tag = "");
...
Circuit<FF>::Circuit(nlohmann::json& circuit_info, Solver* solver, const std::string& tag)
...
// */
//template <typename FF>
//void Circuit<FF>::quadratic_polynomial_handler(barretenberg::fr q_m, barretenberg::fr q_1, barretenberg::fr q_2, barretenberg::fr q_3, barretenberg::fr q_c, uint32_t w){
//    barretenberg::fr b = q_1 + q_2 + q_3;
//
//    if (q_m == 0){
...
//    }
//
//    std::pair<bool, barretenberg::fr> d = (b*b - barretenberg::fr(4) * q_m * q_c).sqrt();
//    if(!d.first){
//        throw std::invalid_argument("There're no roots of quadratic polynomial");
//    }
//    barretenberg::fr x1 = (-b + d.second) / (barretenberg::fr(2) * q_m);
//    barretenberg::fr x2 = (-b - d.second) / (barretenberg::fr(2) * q_m);
//
...
// /**
//  * @brief Relaxes boolean constraints.
//  *
//  * @param w   witness index
//  */
// template <typename FF>
// void Circuit<FF>::boolean_gate_handler(uint32_t w){
//     ((Bool(symbolic_vars[w]) == this->symbolic_vars[0] |
//      (Bool(symbolic_vars[w]) == this->symbolic_vars[1]))).assert_term();
// }
...
template <typename FF> void Circuit<FF>::add_gates()
{
    ...
    for (size_t i = 0; i < get_num_gates(); i++) {
        ...
        for(size_t j = 0; j < this->selectors[i][1].size(); j++){
            B += symbolic_vars[this->wires_idxs[i][1][j]] * this->selectors[i][1][j];
        }
